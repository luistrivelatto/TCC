...
const int LIMITE_CALIBRACAO = 4;
const int MF_THRESHOLD = 10;
...
int main(int argc, char *argv[])
{
    ...
    char filename[20];
    sprintf(filename, "out_mf_cali_teste%s_proc_%d_%d.txt", teste, proc, num_proc);
    ...
    Multiframework_Init_lib(1, TAM+1, num_threads, proc, num_proc, MPI_COMM_WORLD, 1);

    // O trecho que realiza a simulação.
    #pragma omp parallel num_threads(num_threads) default(shared)
    for(int it = 0; it < ITERACOES; it++)
    {
		// Nas primeiras LIMITE_CALIBRACAO iterações, todos os processos MPI compartilham o domínio
		// inteiro, permitindo que a carga de trabalho de cada processo (vetores count e displ) sejam
		// calibrados. Nas demais iterações, os vetores count e displ ficam inalterados.
		if(it < LIMITE_CALIBRACAO)
		{
			int tid = omp_get_thread_num(), ini, fim;

            Multiframework_begin_section(it, count, displ, MF_THRESHOLD, tid, &ini, &fim);
            ...
            // Realiza comunicação coletiva compartilhando o domínio inteiro com todos processos.
            #pragma omp barrier
            #pragma omp single
